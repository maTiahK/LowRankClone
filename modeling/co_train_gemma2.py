...
try:
    ...
    from transformers.models.gemma2.modeling_gemma2 import Gemma2Attention as _BaseGemma2Attention
...
class CustomConfig(Gemma2Config):
...
class AttnWithNewStudentWeight(_BaseGemma2Attention):
    def __init__(self, config: CustomConfig, layer_idx = None):
        ...
        self.student_attn = _BaseGemma2Attention(student_config, layer_idx)
...
class Attn(_BaseGemma2Attention):
    def __init__(self, config: CustomConfig, layer_idx = None):
        super().__init__(config, layer_idx)
        self.config = config
        self.hidden_size = config.hidden_size
...
class AllAttn(_BaseGemma2Attention):
    def __init__(self, config: CustomConfig, layer_idx = None):
        super().__init__(config, layer_idx)
        self.config = config
        self.hidden_size = config.hidden_size
...
class CustomLayer(Gemma2DecoderLayer):
...
class Model(Gemma2Model):
    ...
    def __init__(self, config: CustomConfig):
        ...
        self.layers = nn.ModuleList(
            [CustomLayer(config, layer_idx) if layer_idx not in config.del_layers else Gemma2DecoderLayer(config, layer_idx) 
...
class CoTrainLM(Gemma2ForCausalLM):
